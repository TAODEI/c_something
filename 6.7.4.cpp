...
class Person {
  ...
  public:
    Person(string name, string idNumber, int sex = 0)
        : name(name), idNumber(idNumber), sex(sex) {}
};
...
class Student : virtual protected Person {
  ...
  public:
    Student(string name, string studentID, int grade, string idNumber,
            int sex = 0)
        : Person(name, idNumber, sex), grade(grade), studentID(studentID) {}
};
...
class Teacher : virtual protected Person {
  ...
  public:
    Teacher(string name, string jobTitle, string idNumber, int sex = 0)
        : Person(name, idNumber, sex), jobTitle(jobTitle) {}
};

class StuTeah : public Student, public Teacher {
  public:
    StuTeah(string name, string studentID, int grade, string jobTitle,
            string idNumber, int sex = 0)
